/**
 * Object detection functions. From: https://github.com/tensorflow/tfjs-models/blob/master/coco-ssd/src/index.ts
 */
...
const BASE_PATH = 'file://./src/models/custom/web_model/';
...
export async function load(config = {}) {
  ...
  const base = config.base || 'mobilenet_v2';
...
export class ObjectDetection {
   ...
   *
   *  img The image to classify. Can be a tensor or a DOM element image,
   * video, or canvas.
   *  maxNumBoxes The maximum number of bounding boxes of detected
  ...
  buildDetectedObjects(width, height, boxes, scores, indexes, classes2) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      objects.push({
        bbox: bbox,
        class: classes[classes2[indexes[i]] + 1].displayName,
   ...
   *
   *  img The image to detect objects from. Can be a tensor or a DOM
